...
public final class CustomItems extends ContentContainer {
	...
	public static final CloudyItem CLOUDY_CANDY = new CloudyItem("cloudy_candy",
		new Settings().food(CustomItems.FoodComponents.CLOUDY_CANDY_FOOD).rarity(Rarity.UNCOMMON));
	...
	public static final ArmorMap<DyeableCloudyArmorItem> CLOUD_ARMOR_MAP = new ArmorMap<>("cloudy",
		(rawId, type) -> new DyeableCloudyArmorItem(rawId, CustomItems.ArmorMaterials.CLOUDY, type, new Settings()));
	...
	public static final DyeableMap<CustomBlockItem> DYED_AMETHYST_BLOCK_MAP = new DyeableMap<>("amethyst_block",
		(rawId, color) -> new CustomBlockItem(rawId, CustomBlocks.DYED_AMETHYST_BLOCK_MAP.get(color), new Settings()));
	public static final DyeableMap<CustomBlockItem> DYED_AMETHYST_CLUSTER_MAP = new DyeableMap<>("amethyst_cluster",
		(rawId, color) -> new CustomBlockItem(rawId, CustomBlocks.DYED_AMETHYST_CLUSTER_BLOCK_MAP.get(color),
			new Settings()));

	public static final CustomBlockItem RANDOMIZER_BLOCK = new CustomBlockItem("randomizer", CustomBlocks.RANDOMIZER,
		new Settings().rarity(Rarity.UNCOMMON));

	private static final class FoodComponents {

		public static final FoodComponent CLOUDY_CANDY_FOOD = new FoodComponent.Builder().alwaysEdible().hunger(2)
			.saturationModifier(0.45F).snack()
			.statusEffect(new StatusEffectInstance(StatusEffects.SPEED, 100, 0, false, true, true), 0.25F).build();
	...
	private static final class ArmorMaterials {
		...
		public static final CustomArmorMaterial CLOUDY = CustomArmorMaterial.builder("cloud")
			.setDurability(52, 64, 60, 44).setEnchantability(15).setEquipSound(SoundEvents.ITEM_ARMOR_EQUIP_LEATHER)
			.setProtectionAmount(2, 5, 3, 2).setRepairIngredient(Ingredient.ofItems(CustomItems.CLOUDY_COTTON)).build();
